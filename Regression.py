...
class Linear(Regression):
    def __init__(self, n_iterations=100, learning_rate=0.0001):
        super(Linear, self).__init__(n_iterations=n_iterations,learning_rate=learning_rate,regularization=No_Regulizer())
    ...
    def fit(self, X, y):
        ...
        # We compute the gradient descent
        print(f'Before fine tuning{self.w}')
        for i in range(self.n_iterations):
            y_pred = X.dot(self.w)
            mse=self._loss(y,y_pred) # Loss
            self.training_errors.append(mse)
            # Gradient of l2 loss w.r.t w
            grad_w = -(y - y_pred).dot(X) + self.regularization._grad(self.w)
            # Update the weights
            self.w -= self.learning_rate * grad_w
        print(f'After fine tuning{self.w}')
